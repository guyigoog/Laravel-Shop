...
<main>
  ...
  <script>
      $(document).ready(function() {
          ...
          // fetching products from database and appending them to the page
          function fetch_data() {
              $.ajax({
                  type: "GET",
                  url: "{{ route('jsonfav') }}",
                  ...
                  success: function(response) {
                    console.log(response.productsjs);
                      $('.prod-row').html(""); // Clearing the products box
                      $.each(response.productsjs, function(key, item) { // Looping through the products
                          var id = item.id; // Product ID
                          if (item.favorite == 1) { // If the product is in favorites
                            // Appending the product to the page
                    ...
                    <button class="card-button" type="button">\
                      <div class="card-body">\
                        ...
                        <p></p>\
                          ...
                          } else { // If the product is not in favorites
                            // Appending the product to the page
                              $('.prod-row').append('<div class="prod-card col">\
                  <div class="card h-100">\
                    <button class="fav" value="' + item.id + '">\
